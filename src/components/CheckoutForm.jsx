...
const CheckoutForm = () => {
  ...
  const handlePlaceOrder = async() => {
    try {
      ...
      console.log(placeOrderForm.values)
    ...
    onSubmit: (values) => {
      try {
        if(!values.address || Object.keys(values.address).length === 0)
          dispatch(showSnackbar({ severity:"error", message: "Please add address to place order" }))
        else if(values.products?.length === 0 || !values.userID )
          dispatch(showSnackbar({ severity: "error", message: "Cannot place order. Try again later" }))
        else
          handlePlaceOrder()
      } catch (error) {
        console.log(error)
                ...
                <Box>
                  <FormControl>
                    <RadioGroup
                      ...
                      name="paymentMode"
                      onChange={(e) => {
                        placeOrderForm.setFieldValue("paymentMode", e.target.value)                        
                      }}
                    >
                      <FormControlLabel
                        value="Bank"
